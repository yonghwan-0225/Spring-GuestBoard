...
public class GuessNumberController {
	@GetMapping("/guess")
	public String Guesses(@RequestParam(name = "number", required = false) Integer number, HttpSession session,
			ModelMap model) {
		String message = null;
		...
		if (number == null) {
			...
			if (number < randomNumber) {
				message = "입력 값은 내가 생각하고 있는 숫자보다 작습니다.";
				session.setAttribute("count", ++count);
			} else if (number > randomNumber) {
				message = "입력 값은 내가 생각하고 있는 숫자보다 큽니다.";
				session.setAttribute("count", ++count);
			} else {
